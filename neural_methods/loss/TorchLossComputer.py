...
class TorchLossComputer(object):
    ...
    def cross_entropy_power_spectrum_DLDL_softmax2(inputs, target, Fs, std):
        ...
        fre_distribution = complex_absolute.view(-1)

        #print (sum(complex_absolute.view(-1)), sum(fre_distribution))
        ...
        #plt.plot(complex_absolute[0].detach().cpu().numpy())
        #plt.plot(target_distribution.detach().cpu().numpy())
        #plt.show()

        new_loss = F.nll_loss(torch.log(complex_absolute),target.view((1)).type(torch.long))
        origin_loss = F.cross_entropy(complex_absolute, target.view((1)).type(torch.long))

        return loss_distribution_kl, new_loss,  torch.abs(target[0] - whole_max_idx)
